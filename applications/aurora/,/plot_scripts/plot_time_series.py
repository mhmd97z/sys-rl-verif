...
from aurora_lib.trace import Trace
from aurora_lib.utils import pcc_aurora_reward
...
def plot(trace: Optional[Trace], log_file: str, save_dir: str, cc: str):
    ...
    axes[3].axhline(y=100, color='r', linestyle='--', label='R = 100')
...
def get_best_model(save_dir: str):
    """Get the best model from the validation logs during training."""
    path_to_validation_log = os.path.join(save_dir, "validation_log.csv")
    if not os.path.exists(path_to_validation_log):
        print(path_to_validation_log, 'does not exist!')
        return
    df = pd.read_csv(path_to_validation_log, sep=',')
    assert isinstance(df, pd.DataFrame)
    best_idx = df['mean_validation_reward'].argmax()
    best_step = int(df['num_timesteps'][best_idx])
    best_ckpt = os.path.join(save_dir, "model_step_{}.ckpt".format(best_step))
    print('Best model checkpoint at step =', best_step, 'with mean validation reward =', df['mean_validation_reward'][best_idx])
...
def main():
    ...
    for _, log_file in enumerate(args.log_file):
        ...
        if not args.trace_file:
            ...
            ret_trace = Trace.load_from_file(args.trace_file)
        elif args.trace_file.endswith('.log'):
            ret_trace = Trace.load_from_pantheon_file(args.trace_file, loss=0, queue=10)
        else:
            ret_trace = None
        cc = os.path.basename(log_file).split('_')[0]
        plot(ret_trace, log_file, args.save_dir, cc)

        get_best_model(args.save_dir)
