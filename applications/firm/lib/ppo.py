...
class PPO:
    def __init__(self, env, function_name, data_path, verbose=True):
        self.env = env
    ...
    def train(self, mode='train'):
        ...
        for iteration in range(TOTAL_ITERATIONS):
            ...
            if self.num_same_parameter_critic > MAX_SAME_ITERATIONS and\
                ...
                print('Model parameters are not updated...')
                break
        ...
        if SAVE_TO_FILE:
            file = open(self.data_path + "/ppo_smoothed_rewards.txt", "w")
            for reward in smoothed_rewards:
                file.write(str(reward) + "\n")
            file.close()
            file = open(self.data_path + "/ppo_iteration_rewards.txt", "w")
            for reward in iteration_rewards:
                file.write(str(reward) + "\n")
            file.close()

            # write cpu_utils and slo_preservations to file
            file = open(self.data_path + "/ppo_cpu_utils_all.txt", "w")
            for cpu_util in iteration_cpu_utils:
                file.write(str(cpu_util) + "\n")
            file.close()
            file = open(self.data_path + "/ppo_cpu_utils_smoothed.txt", "w")
            for cpu_util in smoothed_cpu_utils:
                file.write(str(cpu_util) + "\n")
            file.close()
            file = open(self.data_path + "/ppo_slo_preservation_all.txt", "w")
            for ratio in iteration_slo_preservations:
                file.write(str(ratio) + "\n")
            file.close()
            file = open(self.data_path + "/ppo_slo_preservation_smoothed.txt", "w")
            for ratio in smoothed_slo_preservations:
                file.write(str(ratio) + "\n")
            file.close()
        return np.mean(iteration_rewards)
    ...
    def test(self):
        ...
        if SAVE_TO_FILE:
            ...
            file = open(self.data_path + "/ppo_smoothed_rewards_testing.txt", "w")
            for reward in smoothed_rewards:
                file.write(str(reward) + "\n")
            file.close()
            file = open(self.data_path + "/ppo_iteration_rewards_testing.txt", "w")
            for reward in iteration_rewards:
                file.write(str(reward) + "\n")
            file.close()

            # write cpu_utils and slo_preservations to file
            file = open(self.data_path + "/ppo_cpu_utils_all_testing.txt", "w")
            for cpu_util in iteration_cpu_utils:
                file.write(str(cpu_util) + "\n")
            file.close()
            file = open(self.data_path + "/ppo_cpu_utils_smoothed_testing.txt", "w")
            for cpu_util in smoothed_cpu_utils:
                file.write(str(cpu_util) + "\n")
            file.close()
            file = open(self.data_path + "/ppo_slo_preservation_all_testing.txt", "w")
            for ratio in iteration_slo_preservations:
                file.write(str(ratio) + "\n")
            file.close()
            file = open(self.data_path + "/ppo_slo_preservation_smoothed_testing.txt", "w")
            for ratio in smoothed_slo_preservations:
                file.write(str(ratio) + "\n")
            file.close()
        return np.mean(iteration_rewards)
    ...
    def save_checkpoint(self, episode_num):
        checkpoint_name = self.data_path + '/ppo-ep{}.pth.tar'.format(episode_num)
        checkpoint = {
